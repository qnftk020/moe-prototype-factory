...
> **Claude Sonnet + Claude Code CLI** 멀티 에이전트 MoE(Mixture of Experts) 파이프라인으로 **동작하는 프로토타입**을 자동 생성하는 오케스트레이션 대시보드 — **v2.5 Final Release**

아이디어 한 줄이면 **구상 -> 설계 -> MoE 게이팅 -> 병렬 구현 -> MoE 리뷰 -> 피드백**까지 6단계 파이프라인이 자동으로 실행됩니다. 3개의 Sonnet 인스턴스가 프로젝트 복잡도를 분석하고, Claude Code 에이전트 + Stitch 2.0이 병렬로 코드를 구현하며, 3~5명의 Sonnet 전문가가 동시에 리뷰합니다.

**프로토타이핑에 최적화:** 아이디어 검증, MVP 시연, 해커톤, 클라이언트 프레젠테이션 등 빠르게 동작하는 결과물이 필요한 모든 상황에서 활용할 수 있습니다.

**무료 사용:** Claude Max 구독만 있으면 별도 API 비용 없이 로컬에서 실행됩니다.

![Pipeline](https://img.shields.io/badge/Pipeline-6_stages-blue)
![Sonnet](https://img.shields.io/badge/Sonnet_CLI-Planner_&_Reviewer-purple)
...
<p align="center">
  <video src="docs/images/demo.mp4" width="800" controls></video>
</p>

> GitHub에서 영상이 재생되지 않으면 [demo.mp4](docs/images/demo.mp4)를 클릭하세요.
...
| **MoE Gating (AI 게이팅)** | 3개의 Sonnet 인스턴스가 CLAUDE.md의 FE/BE/UI 복잡도를 1~10점으로 평가, 에이전트 활성화 및 모드(Senior/Normal/Skip) 자동 결정 |
| **병렬 구현** | FE Agent + BE Agent가 동시에 코드를 작성, UI는 Stitch 2.0이 담당 |
| **Stitch 2.0 통합** | Google의 AI 디자인 도구가 MCP 프록시를 통해 UI를 생성 -- 기존 UI Agent 대체 |
| **MoE 병렬 리뷰** | 3~5명의 Sonnet 전문가가 동시에 코드를 리뷰, JSON 출력 + 가중 점수 + 중복 제거 |
...
| **Sonnet 출력 정제** | 내부 독백(internal monologue) 자동 제거 -- 깨끗한 결과만 표시 |
...
| Stage | 단계 | 담당 | 병렬 | 설명 |
|-------|------|------|------|------|
| 1 | **Envisioning** | Sonnet x1 | - | 버튼 기반 구조화 Q&A로 요구사항 도출 -> `spec.md` |
| 2 | **Blueprinting** | Sonnet x1 | - | 기획서 -> Claude Code 작업 지침서 `CLAUDE.md` 자동 생성 |
| 3 | **MoE Gating** | Sonnet **x3** | **FE + BE + UI** | CLAUDE.md 복잡도를 AI가 1~10점 평가 -> Agent 활성화/모드 결정 |
| 4 | **Parallel Implementation** | Claude **x2** + Stitch 2.0 | **FE + BE + UI** | Gating 결과에 따라 Senior/Normal/Skip 모드로 병렬 구현 |
| 5 | **MoE Review** | Sonnet **x3~5** | **Experts** | JSON 출력, 가중 점수, 중복 제거로 코드 리뷰 |
...
```
  [Idea Input]
       |
       v
  1. Envisioning (Sonnet x1)
  |  Button-based Q&A (multi-select)
  |  --> spec.md
       |
       v
  2. Blueprinting (Sonnet x1)
  |  spec.md --> CLAUDE.md
       |
       v
  3. MoE Gating (Sonnet x3, parallel)
       +----------+----------+
       |          |          |
     [FE]      [BE]      [UI]
    Score      Score      Score
  ...
  5. MoE Review (Sonnet x3~5, parallel)
       +------+------+------+------+
       |      |      |      |      |
...
CLAUDE.md가 생성되면 3개의 Sonnet 인스턴스가 **병렬로** 프로젝트 복잡도를 분석합니다.
...
3~5개의 Sonnet 인스턴스가 **동시에** 코드를 분석합니다. 코드 기반 Gating으로 필요한 전문가만 활성화됩니다.
...
```bash
# 필수 CLI 도구
# Claude Code CLI만 필요 (Sonnet + Opus 모델 모두 포함)
...
구독 요구사항:
- **Claude Max** -- Claude Code CLI 사용을 위한 Anthropic 구독 (Sonnet + Opus 포함)
...
|-------|-----------|------|
| **AI (Planning)** | Claude Sonnet (`sonnet -p`) | 요구사항 도출, 설계 문서 생성, MoE Gating, MoE 리뷰 |
...
이 프로젝트는 **Claude Sonnet**와 **Claude Code CLI**를 직접 subprocess로 제어합니다. API가 아닌 CLI 도구를 사용하므로, 사용자의 기존 구독만으로 추가 비용 없이 작동합니다.
...
```
   ...
   sonnet -p "prompt"           claude -p "prompt"
...
|----------|--------|-----------|------|
| **Claude Sonnet** | `sonnet -p "prompt"` | `-p` (non-interactive) | 기획, CLAUDE.md 생성, MoE Gating (x3), MoE 리뷰 (x3~5) |
...
### Sonnet Output Cleaning

Claude Sonnet의 출력에는 내부 독백(internal monologue)이 포함될 수 있습니다. 파이프라인은 이를 자동으로 감지하고 제거하여, 구조화된 결과만 다음 단계로 전달합니다.
...
|   |   |-- agents/
|   |   |   |-- sonnet_agent.py       # Claude Sonnet + MoE Gating + 병렬 리뷰
|   |   |   |-- claude_agent.py       # Claude Code + 병렬 구현 (FE/BE)
|   |   |   +-- api_sonnet_agent.py   # Sonnet API 토큰 모드
|   |   +-- requirements.txt
...
Claude Sonnet와 Claude Code CLI는 각각 자체 인증을 사용합니다:

```bash
# Claude Sonnet -- Google Cloud 인증
sonnet    # 최초 실행 시 브라우저로 로그인
...
- [Claude Code](https://github.com/anthropics/claude-code) -- Anthropic (Sonnet + Opus)
